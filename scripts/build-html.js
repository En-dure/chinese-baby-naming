...
.hero{
  ...
  background:
    radial-gradient(ellipse at 20% 50%,rgba(168,54,44,.14),transparent 55%),
    radial-gradient(ellipse at 85% 60%,rgba(156,122,53,.12),transparent 50%),
    linear-gradient(135deg,#241f1a 0%,#3a3026 55%,#2b241d 100%);
  color:#f3ead9;padding:2.4rem 2rem 2rem;text-align:center;
}
...
.hero .seal{
  background:var(--cinnabar);color:#f5ede0;font-family:var(--brush);
  width:3.2rem;height:3.2rem;display:flex;align-items:center;justify-content:center;
  font-size:1.5rem;border-radius:7px;transform:rotate(-4deg);
  box-shadow:0 0 0 2px rgba(245,237,224,.15) inset,0 4px 12px rgba(0,0,0,.25);
  position:relative;
}
...
.sidebar{
  width:266px;flex-shrink:0;position:sticky;top:1.4rem;
  max-height:calc(100vh - 2.8rem);overflow-y:auto;
  display:flex;flex-direction:column;gap:.85rem;padding-right:4px;
}
...
.card{
  background:var(--card);border-radius:var(--r);padding:1rem 1.15rem;
  box-shadow:var(--sh1);border:1px solid var(--line2);
}
.card h3{
  font-family:var(--serif);font-weight:600;font-size:.88rem;color:var(--ink);
...
.bz-input{
  width:100%;font-family:var(--serif);font-size:.92rem;
  padding:.42rem .55rem;border:1px solid var(--line);border-radius:var(--rs);
...
.bz-pillars .pz{
  flex:1;font-family:var(--serif);font-weight:500;font-size:.92rem;
  padding:.28rem .2rem;border-radius:6px;background:var(--card2);color:var(--ink);
  text-align:center;border:1px solid var(--line2);transition:all .2s;
}
...
.chip{
  padding:.26rem .62rem;border-radius:16px;border:1px solid var(--line);
  background:var(--paper);cursor:pointer;font-size:.74rem;color:var(--ink2);
...
.main-top{
  ...
  border-radius:var(--r);padding:1.2rem 1.5rem;box-shadow:var(--sh2);
...
.name-assemble .slot{
  font-family:var(--serif);font-weight:500;font-size:2.1rem;width:2.6rem;
  text-align:center;border:none;border-bottom:2px solid var(--line);
  background:transparent;color:var(--ink);cursor:pointer;transition:all .2s;line-height:1.1;
}
...
.wg{
  text-align:center;padding:.32rem .5rem;background:var(--paper);
  border-radius:6px;min-width:2.9rem;border:1px solid var(--line2);transition:all .2s;
}
...
.name-card{
  ...
  background:linear-gradient(135deg,var(--card),var(--card2));
  border-radius:var(--r);padding:.7rem 1.3rem;box-shadow:var(--sh3);
  margin-right:.6rem;border:1px solid var(--gold3);
  position:relative;
}
.name-card::before{content:'';position:absolute;left:0;top:0;bottom:0;width:3px;background:var(--cinnabar);border-radius:3px 0 0 3px}
.name-card .full-name{font-family:var(--serif);font-weight:600;font-size:1.4rem;letter-spacing:.18rem;color:var(--ink)}
.name-card .wuge-sum{font-size:.7rem;color:var(--ink3)}
.name-card .all-ji{color:var(--jade);font-weight:700;font-family:var(--serif);margin-left:.3rem}
...
.btn.pri{background:var(--cinnabar);color:#fff;border-color:var(--cinnabar)}
.btn.pri:hover{background:var(--cinnabar2)}
...
.cell{
  ...
  padding:.6rem .2rem .45rem;text-align:center;
...
.cell .wx-dot.金{background:var(--gold)}
.cell .wx-dot.木{background:var(--jade)}
.cell .wx-dot.水{background:var(--indigo)}
.cell .wx-dot.火{background:var(--cinnabar)}
.cell .wx-dot.土{background:#8b5a2b}

/* ===== 详情弹窗 ===== */
.detail-overlay{
  position:fixed;inset:0;background:rgba(38,34,29,.45);
...
.detail-box{
  ...
  padding:0;box-shadow:0 12px 48px rgba(38,34,29,.25);position:relative;
  overflow:hidden;animation:popIn .28s cubic-bezier(.34,1.56,.64,1);
}
@keyframes popIn{from{opacity:0;transform:scale(.92) translateY(8px)}to{opacity:1;transform:scale(1) translateY(0)}}
.detail-box .db-top{
  background:linear-gradient(135deg,#3a3026,#241f1a);color:#f3ead9;
  padding:1.8rem 2rem 1.4rem;text-align:center;position:relative;
}
...
.detail-box .source-quote{
  margin-top:1rem;padding:.75rem 1rem;background:var(--paper2);
  border-radius:var(--rs);border-left:3px solid var(--gold2);
  font-family:var(--serif);font-size:.82rem;line-height:1.8;color:var(--ink2);
}
...
.empty::before{content:'';display:block;font-size:2rem;margin-bottom:.5rem;opacity:.3}

@media(max-width:920px){
  .wrap{flex-direction:column;padding:1rem}
  .sidebar{width:100%;position:relative;max-height:none;flex-direction:row;flex-wrap:wrap;padding-right:0}
  .sidebar .card{flex:1;min-width:150px}
...
<aside class="sidebar">
  ...
  <div class="card">
    <h3>笔画 <span class="en">strokes</span></h3>
    <div class="chips" id="strokeChips"></div>
  </div>
  <div class="card">
    <h3>五行 <span class="en">wuxing</span></h3>
    <div class="chips" id="wxChips"></div>
  </div>
  <div class="card">
    <h3>风格 <span class="en">style</span></h3>
    <div class="chips" id="styleChips"></div>
  </div>
  <div class="card">
    <h3>性别 <span class="en">gender</span></h3>
  ...
  <div class="card">
    <h3>排除 <span class="en">exclude</span></h3>
    <div class="chips" id="excludeChips"></div>
  </div>
  <div class="card">
    <h3>五格大吉组合 <span class="en">wuge</span></h3>
    <div class="combo-note">
      刘 = 15画（康熙）<br>
      <span class="star">★</span> <b>10 + 14</b> 全吉最优<br>
...
function calcBazi(){
  ...
  for(let i=JIEQI.length-1;i>=0;i--){
    if(dateStr>=JIEQI[i].d){monthZhi=JIEQI[i].z;break;}
  }
  ...
  let hourPillar='——';
  if(hour>=0){
    const ziGan=((dayGanIdx%5)*2)%10;
    const hourGanIdx=(ziGan+hour)%10;
  ...
  pzs.forEach((p,i)=>{
    const hasVal = (i<3) || (i===3 && hour>=0);
    p.className='pz'+(hasVal?' has-val':' empty');
  });
  ...
  xy.innerHTML='日主 <b>'+dayGan+'('+dayWx+')</b> 生于'+monthZhiStr+'月，<b>'+strength+'</b><br>'+
    '五行：'+wxc+'<br>'+
    '喜用：<b>'+xi+'</b> · 忌：<b style="color:var(--cinnabar)">'+ji+'</b><br>'+
    '<span style="color:var(--ink3);font-size:.66rem">※ 喜用为程序粗判，精确需命理师综合四柱</span>';
